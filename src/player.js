export default class extends Phaser.GameObjects.Sprite {
  constructor(scene, x, y, multiplayer) {
    super(scene, x, y, "atlas", "elf_m_hit_anim_f0.png").setOrigin(0, 0.5);
    ...
    if (!this.movingRight) {
      this.scene.tweens.add({
        ...
        onStart(tween, targets) {
          ...
          if (cursors) player.multiplayer.playerMoveRight(player.userId);
        },
        onComplete(tween, targets) {
          const player = targets[0];
          player.movingRight = false;
          if (!cursors) {
            console.log("onComplete move right");
    ...
    if (!this.movingLeft) {
      this.scene.tweens.add({
        ...
        onStart(tween, targets) {
          ...
          if (cursors) player.multiplayer.playerMoveLeft(player.userId);
        },
        onComplete(tween, targets) {
          const player = targets[0];
          ...
          if (!cursors) {
            player.anims.play("m_idle", true);
            console.log("onComplete move left");
          } else if (!cursors.left.isDown && !cursors.right.isDown && !cursors.up.isDown)
            player.anims.play("m_idle", true);
        },
      });
